...
func TestLoadHTTPIngressSafetyDefaults(t *testing.T) {
	...
	require.Zero(t, cfg.Gateway.ResponsesMaxBodySize)
...
func TestLoadDefaultRuntimeIdentityDoesNotExposeLegacyBrand(t *testing.T) {
	resetViperWithJWTSecret(t)
	...
	require.Equal(t, "gateway", cfg.Log.ServiceName)
	require.Equal(t, "gateway", cfg.Database.DBName)
	require.Equal(t, "gateway:", cfg.Dashboard.KeyPrefix)
}
...
func TestLoadDefaultDashboardCacheConfig(t *testing.T) {
	...
	if cfg.Dashboard.KeyPrefix != "gateway:" {
		t.Fatalf("Dashboard.KeyPrefix = %q, want %q", cfg.Dashboard.KeyPrefix, "gateway:")
	}
...
func TestValidateConfigErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:    "gateway responses body negative",
			mutate:  func(c *Config) { c.Gateway.ResponsesMaxBodySize = -1 },
			wantErr: "gateway.responses_max_body_size",
		},
		{
			name:    "gateway responses body exceeds global body limit",
...
func TestLoad_DefaultGatewayImageStreamConfig(t *testing.T) {
	...
	if cfg.Gateway.StreamKeepaliveInterval != 0 {
		t.Fatalf("stream_keepalive_interval = %d, want 0", cfg.Gateway.StreamKeepaliveInterval)
	}
