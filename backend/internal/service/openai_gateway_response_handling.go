...
func (s *OpenAIGatewayService) handleStreamingResponseWithReasoning(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string) (*openaiStreamingResult, error) {
	...
	processSSELine := func(line string, queueDrained bool) {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			// A protocol-progress event proves the upstream is alive, but it is not
			// client-visible output. Stop the first-output timeout separately from
			// TTFT so response.created/reasoning scaffolding cannot report 0 ms.
			if !guardFirstOutput && startsClientOutput {
				StreamAttemptMarkFirstSemanticEvent(c)
				firstOutputProgressObserved = true
				stopFirstOutputTimer()
			}
