...
func cachedOpenAIPromptCacheKey(c *gin.Context) string {
	if c == nil {
		return ""
	}

	cached, ok := c.Get(OpenAIPromptCacheKey)
	if !ok {
		return ""
	}
	sessionID, ok := cached.(string)
	if !ok {
		return ""
	}
...
func explicitOpenAISessionID(c *gin.Context, body []byte) string {
	...
	if sessionID == "" {
		sessionID = cachedOpenAIPromptCacheKey(c)
	}
