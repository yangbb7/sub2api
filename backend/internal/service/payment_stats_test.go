...
func TestComputeBasicStatsGroupsAmountsByCurrency(t *testing.T) {
	...
	computeBasicStats(stats, orders, todayStart, todayStart.AddDate(0, 0, 1))
...
func TestPaymentDashboardBreakdownsGroupAmountsAndRankingsByCurrency(t *testing.T) {
	...
	start := time.Date(2026, time.July, 24, 0, 0, 0, 0, time.UTC)
	daily := buildDailySeries(orders, start, start.AddDate(0, 0, 2), time.UTC)
