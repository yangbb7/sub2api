...
// UserTokenVersionRepository atomically advances the persistent token epoch.
// It is kept narrow so token revocation does not depend on a stale User snapshot.
...
func (s *UserService) ChangePassword(ctx context.Context, userID int64, req ChangePasswordRequest) error {
	...
	// The repository updates the password hash and atomically advances TokenVersion.
