...
func TestResolveOpenAIUpstreamEndpoint_AlphaSearchResponsesFallback(t *testing.T) {
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, EndpointAlphaSearch, nil)
	c.Set(ctxKeyInboundEndpoint, EndpointAlphaSearch)
...
func TestResolveRuntimeUpstreamEndpointPrefersRuntimeSelection(t *testing.T) {
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, EndpointAlphaSearch, nil)
	c.Set(ctxKeyInboundEndpoint, EndpointAlphaSearch)
	...
	require.Equal(t, EndpointResponses, resolveRuntimeUpstreamEndpoint(c, service.PlatformOpenAI))
}
