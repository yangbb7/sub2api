...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	if platform == service.PlatformGemini {
		...
		for {
			...
			requestSnapshot := service.BuildUsageRequestSnapshot(body)
	...
	for {
		...
		for {
			...
			requestSnapshot := service.BuildUsageRequestSnapshot(attemptParsedReq.Body.Bytes())
			responseSnapshot := result.ResponseSnapshot
			inboundEndpoint := GetInboundEndpoint(c)
			upstreamEndpoint := GetUpstreamEndpoint(c, account.Platform)
...
func normalizeAntigravityClaudeModelRequest(body []byte, parsedReq *service.ParsedRequest) ([]byte, error) {
	if parsedReq == nil {
		return body, nil
	}
