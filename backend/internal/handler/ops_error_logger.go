...
func OpsErrorLoggerMiddleware(ops *service.OpsService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if status < 400 {
			...
			recoveredSeverityStatus := effectiveUpstreamStatus
			if isOpsContextWindowExceeded(c, recoveredMsg, "") {
				recoveredErrorType = "invalid_request_error"
				recoveredSeverityStatus = http.StatusBadRequest
			}
			...
			entry := &service.OpsInsertErrorLogInput{
				...
				UpstreamEndpoint: resolveRuntimeUpstreamEndpoint(c, platform),
				...
				Severity:          classifyOpsSeverity(recoveredErrorType, recoveredSeverityStatus),
				StatusCode:        status,
