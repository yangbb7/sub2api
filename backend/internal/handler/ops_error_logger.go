...
func classifyOpsErrorLog(c *gin.Context, errType, message, code string, status int) (phase string, isBusinessLimited bool, errorOwner string, errorSource string) {
	...
	if contextWindowLimited {
		phase = "request"
	} else if localModelConfiguration {
		phase = "routing"
	...
	if clientBusinessLimited && !upstreamError && !routingCapacityLimited && !contextWindowLimited && !localModelConfiguration {
		phase = "auth"
	}
	...
	msg := strings.ToLower(message)
	effectiveUpstreamError := upstreamError && !localModelConfiguration && !contextWindowLimited
	...
	isBusinessLimited = contextWindowLimited || localModelConfiguration || routingCapacityLimited || (clientBusinessLimited && !effectiveUpstreamError) || localBusinessLimited
...
func isOpsContextWindowExceeded(c *gin.Context, message, code string) bool {
	...
	if value, ok := c.Get(service.OpsUpstreamErrorMessageKey); ok {
		if upstreamMessage, ok := value.(string); ok && service.IsOpenAIContextWindowError(upstreamMessage, nil) {
			return true
		}
	}
