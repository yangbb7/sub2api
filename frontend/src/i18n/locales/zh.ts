export default {
  ...
  auth: {
    ...
    invitationCodePlaceholder: '请输入注册邀请码',
    ...
    affiliateCodeLabel: '邀请返佣码',
    affiliateCodePlaceholder: '输入邀请返佣码（可选）',
    affiliateReferralApplied: '已通过邀请链接加入，邀请返佣码：{code}',
  ...
  errors: {
    ...
    pageNotFound: '页面未找到',
    pageNotFoundDescription: '该页面不存在，或已经被移动。',
    unauthorized: '未授权',
    forbidden: '禁止访问',
    serverError: '服务器错误',
    networkError: '网络错误',
  ...
  admin: {
    ...
    accounts: {
      ...
      crsBaseUrl: 'CRS 服务地址',
      crsBaseUrlPlaceholder: '例如：http://127.0.0.1:13000',
      crsUsername: '用户名',
      crsPassword: '密码',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个 gateway 实例的场景。',
    ...
    ops: {
      ...
      diagnosis: {
        ...
        diskCritical: '磁盘使用率严重过高 ({usage}%)',
        diskCriticalImpact: '可能导致数据库写入、日志、构建缓存或容器层写入失败',
        diskCriticalAction: '清理旧镜像、日志和临时文件，必要时扩容磁盘',
        diskHigh: '磁盘使用率偏高 ({usage}%)',
        diskHighImpact: '剩余空间不足会降低部署和数据库写入安全边际',
        diskHighAction: '检查磁盘增长趋势，清理无用镜像和历史日志',
      ...
      tooltips: {
        ...
        disk: '磁盘使用率，包括网关运行环境当前可见的已用和总容量。',
    ...
    settings: {
      ...
      payment: {
        ...
        maxAmount: '最高金额',
        dailyLimit: '每日限额',
        balanceRechargeMultiplier: 'CNY 入账 USD 倍率',
        balanceRechargeMultiplierHint: 'CNY 支付渠道的默认入账倍率；服务商可单独覆盖，加密钱包默认 1 USDC = 1 USD',
        ...
        field_currency: '支付币种',
        field_creditRateToUsd: '入账 USD 倍率',
        ...
        field_coinbaseApiBaseHint: '默认 Coinbase Business API：https://business.coinbase.com/api/v1；只有沙箱、代理或本地测试时才需要改。',
        field_coinbaseCurrencyHint: '建议使用 USDC。若选择 USD 等法币，Coinbase 会按其收银台规则换算和结算。',
        field_creditRateToUsdHint: '该服务商每收 1 单位支付币种，给用户入账多少 USD 余额。留空时 CNY 使用全局倍率，USD/USDC 默认 1。',
        ...
        coinbaseWebhookHint: '请在 Coinbase Business 后台将以下地址配置为 Webhook 端点；订阅 checkout.payment.success、checkout.payment.failed、checkout.payment.expired，退款事件按需订阅。',
