...
assert_line Dockerfile.goreleaser 'COPY --chown=gateway:gateway backend/resources /app/resources'
assert_line deploy/Dockerfile 'COPY --from=backend-builder --chown=gateway:gateway /app/backend/resources /app/resources'
