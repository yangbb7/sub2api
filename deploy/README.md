# AI Gateway Deployment Files

This directory contains files for deploying AI Gateway on Linux servers and Apple-silicon Macs.
...
| `gateway.service` | Systemd service unit file |
| `gateway-datamanagementd.service` | datamanagementd systemd service unit file |
...
curl -sSL https://raw.githubusercontent.com/yangbb7/sub2api/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs gateway | grep "admin password"
   ```
...
| `ADMIN_EMAIL` | No | `admin@gateway.local` | Admin email |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf gateway-complete.tar.gz deployment/

# Transfer to new server
scp gateway-complete.tar.gz user@new-server:/path/to/destination/
...
AI Gateway supports three methods to connect to Gemini:
...
1. Download the latest release from [GitHub Releases](https://github.com/yangbb7/sub2api/releases)
2. Extract and copy the binary to `/opt/gateway/`
3. Copy `gateway.service` to `/etc/systemd/system/`
4. Run:
   ```bash
...
The main config file is at `/etc/gateway/config.yaml` (created by Setup Wizard).
...
/opt/gateway/
├── gateway              # Main binary
├── gateway.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 gateway
...
# Check config file
sudo cat /etc/gateway/config.yaml
...
AI Gateway supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> **Tip:** Use a trusted TLS fingerprint inspection tool to compare device and browser profiles before adding custom entries.
