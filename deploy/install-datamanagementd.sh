...
# 或：
#   sudo ./install-datamanagementd.sh --source /path/to/gateway/repo

BIN_PATH=""
SOURCE_PATH=""
INSTALL_DIR="/opt/gateway"
DATA_DIR="/var/lib/gateway/datamanagement"
SERVICE_FILE_NAME="gateway-datamanagementd.service"
...
参数:
  ...
  --source  指定 gateway 仓库路径（脚本会执行 go build）
  -h, --help 显示帮助

示例:
  sudo ./install-datamanagementd.sh --binary ./datamanagement/datamanagementd
  sudo ./install-datamanagementd.sh --source /opt/gateway-src
EOF
}
...
systemctl enable --now gateway-datamanagementd

echo "[6/6] 完成，当前状态："
systemctl --no-pager --full status gateway-datamanagementd || true

cat <<'EOF'

下一步建议：
1. 查看日志：sudo journalctl -u gateway-datamanagementd -f
2. 在 gateway（容器部署时）挂载 socket:
