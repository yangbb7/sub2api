...
# AI Gateway 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/yangbb7/sub2api/main/deploy/install.sh | bash
#
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="AI Gateway 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 AI Gateway 服务："
    ...
    ["not_installed"]="AI Gateway 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 AI Gateway。"
    ...
    ["uninstall_complete"]="AI Gateway 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 AI Gateway"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 AI Gateway"
    ...
    ["server_config_desc"]="配置 AI Gateway 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="AI Gateway Installation Script"
    ...
    # Completion
    ["install_complete"]="AI Gateway installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start AI Gateway service:"
    ...
    # Upgrade
    ["upgrading"]="Upgrading AI Gateway..."
    ...
    ["not_installed"]="AI Gateway is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove AI Gateway from your system."
    ...
    ["uninstall_complete"]="AI Gateway has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install AI Gateway"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove AI Gateway"
    ...
    ["server_config_desc"]="Configure AI Gateway server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/gateway" ]; then
        ...
        "$INSTALL_DIR/gateway" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="gateway_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/gateway" "$INSTALL_DIR/gateway"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/gateway"
}
...
[Unit]
Description=AI Gateway - AI API Gateway Platform
Documentation=https://github.com/yangbb7/sub2api
...
start_service() {
    ...
    if systemctl start gateway; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u gateway -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable gateway 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status gateway"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u gateway -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart gateway"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop gateway"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if AI Gateway is installed
    if [ ! -f "$INSTALL_DIR/gateway" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/gateway" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    ...
    cp "$INSTALL_DIR/gateway" "$INSTALL_DIR/gateway.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/gateway.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/gateway"
...
# Requires: AI Gateway must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/gateway" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="gateway.backup.${current_version}"
        else
            backup_name="gateway.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/gateway" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    systemctl disable gateway 2>/dev/null || true
